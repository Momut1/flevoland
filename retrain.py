  ...
  if how_many >= 0:
    ...
    for label_index, label_name in enumerate(image_lists.keys()):
      for image_index, image_name in enumerate(
        ...
        ground_truths.append(label_index)
        filenames.append(image_name)
  #print("the batches used are: {}".format(filenames))
...
def add_final_retrain_ops(class_count, final_tensor_name, bottleneck_tensor,
                          quantize_layer, is_training, contrib_quantize=None):
...
def run_final_eval(train_session, module_spec, class_count, image_lists,
  ...
  COLUMN_NAMES = ['image', 'class_pred', 'gt']
  ...
  if FLAGS.print_misclassified_test_images:
    ...
    for i, test_filename in enumerate(test_filenames):
      if predictions[i] != test_ground_truth[i]:
        ...
        df.loc[len(df)] = [test_filename, predictions[i], test_ground_truth[i]]

  df.to_csv(FLAGS.summaries_dir + "missclass.csv")
...
def main(_):
  ...
  logging.info('%s Hub file name.', FLAGS.tfhub_module)
  ...
  with tf.Session(graph=graph) as sess:
    ...
    if FLAGS.train_from_ckpt != None:
        # set the weights of the model from a checkpoint of a trainning
        tf.train.Saver().restore(sess, FLAGS.train_from_ckpt)
    else:
    ...
    for i in range(FLAGS.how_many_training_steps):
      #TODO add flag for adaptative larning rates
    ...
    #Add a check to see if we already have a save model, if TRUE then erase it so we can write it again
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      type=str,
      default=None,
      help='Path to checkpoint from where you want to start the retrain'
  )
  ...
  parser.add_argument(
      ...
      type=str,
      default=None,
      help='Adam optimizer or Gradient descent'
  )
